...
FROM quantumobject/docker-baseimage:18.04 AS base
MAINTAINER Angel Rodriguez  "angel@quantumobject.com"

# Update the container
# Installation of nesesary package/software for this containers...
RUN apt-get -y update
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y -q --no-install-recommends apt-utils \
                    git \
                    bzip2 \
                    unzip \
...
WORKDIR /tmp
RUN wget -q https://julialang-s3.julialang.org/bin/linux/x64/`echo ${JULIA_VERSION} | cut -d. -f 1,2`/julia-${JULIA_VERSION}-linux-x86_64.tar.gz
RUN echo "be7af676f8474afce098861275d28a0eb8a4ece3f83a11027e3554dcdecddb91 *julia-${JULIA_VERSION}-linux-x86_64.tar.gz" | sha256sum -c -
